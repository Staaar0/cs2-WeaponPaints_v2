...
public partial class WeaponPaints
{
	...
	private void RefreshPlayerGlovesAfterChatInput(CCSPlayerController player)
	{
		...
		AddTimer(0.05f, () => GivePlayerGloves(player, refreshGlovesAfterKnife: true), TimerFlags.STOP_ON_MAPCHANGE);
	}
	...
	{
		var paintInfo = SkinsList.Concat(GlovesList).FirstOrDefault(item =>
			TryReadInt(item["weapon_defindex"]) == weaponDefIndex &&
			TryReadInt(item["paint"]) == paintId);

		if (!GenPaintMetadata.TryGetValue((weaponDefIndex, paintId), out var metadata))
			return paintInfo;

		if (paintInfo == null)
			return metadata;

		if (paintInfo["rarity"] != null)
			return paintInfo;

		var merged = (JObject)paintInfo.DeepClone();
		merged["rarity"] = metadata["rarity"]?.DeepClone();
		return merged;
	}
	...
	private void ApplyGloveSelection(CCSPlayerController? player, JObject selectedGlove)
	{
			...
			foreach (var team in teamsToCheck)
		{
				...
				{
					SavePaintCustomization(player.Slot, team, weaponDefindex, weaponInfo.Paint, weaponInfo);
				}
	...
	private void OpenStickerSourceMenu(CCSPlayerController? player, int stickerSlot)
	{
		...
		foreach (var source in StickerSources)
		{
			...
			{
				sourceMenu.AddMenuOption(sourceName, (menuPlayer, _) => OpenStickerTypeMenu(menuPlayer, stickerSlot, sourceName, string.Empty));
			}
			else
			{
	...
	private void OpenStickerEventMenu(CCSPlayerController? player, int stickerSlot, string stickerSource)
	{
		...
		{
			var groupName = eventGroup;
			eventMenu.AddMenuOption(groupName, (menuPlayer, _) => OpenStickerTypeMenu(menuPlayer, stickerSlot, stickerSource, groupName));
		}
	...
	private void OpenStickerTypeMenu(CCSPlayerController? player, int stickerSlot, string stickerSource, string stickerGroup)
	{
		if (!Utility.IsPlayerValid(player) || player is null) return;

		var typeMenu = Utility.CreateMenu(Localizer["wp_sticker_type_menu_title", string.IsNullOrEmpty(stickerGroup) ? stickerSource : stickerGroup]);
		...
		{
			if (stickerSource == "Community / Workshop Stickers")
			{
				OpenStickerSourceMenu(menuPlayer, stickerSlot);
			}
			else
		...
		foreach (var stickerType in stickerTypes)
		{
			...
			typeMenu.AddMenuOption(typeName, (menuPlayer, _) =>
			{
				...
				{
					OpenStickerListMenu(menuPlayer, stickerSlot, stickerSource, stickerGroup, typeName, string.Empty);
				}
			});
		}
	...
	private void OpenStickerAudienceMenu(CCSPlayerController? player, int stickerSlot, string stickerSource, string stickerGroup, string stickerType)
	{
		if (!Utility.IsPlayerValid(player) || player is null) return;
		...
		audienceMenu.AddMenuOption(BackMenuLabel, (menuPlayer, _) => OpenStickerTypeMenu(menuPlayer, stickerSlot, stickerSource, stickerGroup));
		audienceMenu.AddMenuOption(Localizer["wp_sticker_teams"], (menuPlayer, _) => OpenStickerListMenu(menuPlayer, stickerSlot, stickerSource, stickerGroup, stickerType, "Teams"));
		audienceMenu.AddMenuOption(Localizer["wp_sticker_players"], (menuPlayer, _) => OpenStickerListMenu(menuPlayer, stickerSlot, stickerSource, stickerGroup, stickerType, "Players"));
	...
	private void OpenStickerListMenu(CCSPlayerController? player, int stickerSlot, string stickerSource, string stickerGroup, string stickerType, string stickerAudience)
	{
		if (!Utility.IsPlayerValid(player) || player is null) return;
		...
		stickerMenu.AddMenuOption(BackMenuLabel, (menuPlayer, _) =>
		{
			if (string.IsNullOrEmpty(stickerAudience))
			{
				OpenStickerTypeMenu(menuPlayer, stickerSlot, stickerSource, stickerGroup);
			}
			else
			{
				OpenStickerAudienceMenu(menuPlayer, stickerSlot, stickerSource, stickerGroup, stickerType);
			}
		});
	...
	{
		if (!string.IsNullOrWhiteSpace(GetStickerEvent(sticker))) return "Major Stickers";
		if (!string.IsNullOrWhiteSpace(GetStickerOperationGroup(sticker))) return "Operation Stickers";
		if (!string.IsNullOrWhiteSpace(GetStickerCapsuleGroup(sticker))) return "Capsule Stickers";

		return "Community / Workshop Stickers";
	}

	private static string GetStickerEvent(JObject sticker)
	{
		var tournamentEvent = sticker["tournament_event"]?.ToString();
		if (!string.IsNullOrWhiteSpace(tournamentEvent)) return tournamentEvent;

		// tournament stickers are named "Sticker | Team or Player | Event"
		var name = GetStickerMenuName(sticker["name"]?.ToString() ?? string.Empty);
		var parts = name.Split('|', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
		if (parts.Length < 2) return string.Empty;

		var eventName = parts[^1];
		var index = Array.FindIndex(MajorStickerEvents, knownEvent => string.Equals(knownEvent, eventName, StringComparison.OrdinalIgnoreCase));
		return index >= 0 ? MajorStickerEvents[index] : string.Empty;
	}

	private static string GetStickerEventGroup(JObject sticker, string stickerSource)
	{
		return stickerSource switch
		{
	...
	private static bool StickerMatchesGroup(JObject sticker, string stickerSource, string stickerGroup)
	{
		if (string.IsNullOrEmpty(stickerGroup)) return true;

		return string.Equals(GetStickerEventGroup(sticker, stickerSource), stickerGroup, StringComparison.OrdinalIgnoreCase);
	}

	private static int GetStickerEventMenuOrder(string stickerSource, string stickerGroup)
	{
		var knownEvents = stickerSource switch
		{
		...
		var index = Array.FindIndex(knownEvents, knownEvent => string.Equals(knownEvent, stickerGroup, StringComparison.OrdinalIgnoreCase));
		return index >= 0 ? index : knownEvents.Length;
	}

	private static string GetStickerType(string? stickerName)
	{
	...
	private static string GetStickerOperationGroup(JObject sticker)
	{
		var capsule = sticker["capsule"]?.ToString() ?? string.Empty;
		var name = sticker["name"]?.ToString() ?? string.Empty;

		foreach (var operation in OperationStickerEvents)
		{
			if (capsule.Contains(operation, StringComparison.OrdinalIgnoreCase) ||
			    name.Contains(operation, StringComparison.OrdinalIgnoreCase)) return operation;
		}

		var subject = GetStickerSubject(name);
		if (ShatteredWebStickerSubjects.Contains(subject, StringComparer.OrdinalIgnoreCase)) return "Shattered Web";
		if (RiptideStickerSubjects.Contains(subject, StringComparer.OrdinalIgnoreCase)) return "Riptide";
		if (BrokenFangStickerSubjects.Contains(subject, StringComparer.OrdinalIgnoreCase)) return "Broken Fang";

		return string.Empty;
	}

	private static string GetStickerCapsuleGroup(JObject sticker)
	{
		var capsule = sticker["capsule"]?.ToString() ?? string.Empty;
		...
		{
			var capsuleLabel = capsule
				.Replace("Sticker Capsule", string.Empty, StringComparison.OrdinalIgnoreCase)
				.Replace("Sticker Collection", string.Empty, StringComparison.OrdinalIgnoreCase)
				.Trim();
			return string.IsNullOrWhiteSpace(capsuleLabel) ? capsule : capsuleLabel;
		}

		var subject = GetStickerSubject(sticker["name"]?.ToString() ?? string.Empty);
		if (StickerCapsule2Subjects.Contains(subject, StringComparer.OrdinalIgnoreCase)) return "Sticker Capsule 2";
		if (AmbushStickerSubjects.Contains(subject, StringComparer.OrdinalIgnoreCase)) return "Ambush";

		return string.Empty;
	}

	private static bool IsPlayerSticker(JObject sticker)
	{
		if (!string.IsNullOrWhiteSpace(sticker["tournament_player"]?.ToString())) return true;

		var subject = GetStickerSubject(sticker["name"]?.ToString() ?? string.Empty);
		if (string.IsNullOrWhiteSpace(subject)) return false;

		if (StickerTeamNames.Any(team => string.Equals(subject, team, StringComparison.OrdinalIgnoreCase))) return false;
		if (MajorStickerEvents.Any(knownEvent => string.Equals(subject, knownEvent, StringComparison.OrdinalIgnoreCase))) return false;
	...
	private void ApplyRemoveAllStickers(CCSPlayerController? player, ChatMenuOption option)
	{
		...
		var wearKey = (weaponDefIndex, weaponInfo.Paint);
		var clearWearDelta = StickerRenderWearStep / 2.0f;
		var clearWear = weaponInfo.Wear + clearWearDelta <= 1.0f
			? weaponInfo.Wear + clearWearDelta
			: Math.Max(0.0f, weaponInfo.Wear - clearWearDelta);
		playerWear[wearKey] = clearWear;

		if (weapon != null && weapon.IsValid)
		{
			GivePlayerWeaponSkin(player, weapon);
			weapon.FallbackWear = clearWear;
		}
	...
	private void RefreshKnifeEntityForGloves(CCSPlayerController player, bool refreshGloveState = false)
	{
		...
		AddTimer(0.06f, () =>
		{
			...
			Server.NextFrame(() =>
			{
				...
				if (refreshGloveState)
				{
					var currentPawn = currentPlayer.PlayerPawn.Value;
					if (currentPawn != null && currentPawn.IsValid) TouchGloveState(currentPawn);
				}
			});
		}, TimerFlags.STOP_ON_MAPCHANGE);
	}
